...
from pygame_tools.UI.ui import Button
...
class Display(State):
    def __init__(self, game):
        ...
        self.resolution = pygame.Vector2(self.game.screen_resolution)
        self.quality = pygame.Vector2(self.game.game_resolution)
        ...
        self.menu_options = {0: "Aspect Ratio", 1: "Resolution", 2: "Game Quality",
                             3: "HUD Scale", 4: "Full Screen", 5: "Target FPS"}

        self.element_options = {0: self.aspect, 1: self.resolution, 2: self.quality,
                                3: self.gui_scale, 4: self.fullscreen, 5: self.FPS}
...
# METHODS ---------------------------------
    ...
    def __create_menu_options__(self):
        ...
        """
        test1 = Button((200, 300), (self.quality.x/2, self.quality.y/2), 1, "hallo")

        title_height = self.quality.y / 6  # Menu title height

        self.spacing = -4  # Change to adjust the height of each menu option

        # Menu options surface height
        options_height = (self.quality.y / len(self.menu_options) + self.spacing) * (len(self.menu_options) - 1)
        # Variables for the cursor
        self.cursor_img = pygame.image.load(os.path.join(self.game.assets_dir, "map", "cursor.png")).convert()
        self.cursor_pos_x, self.cursor_pos_y = self.quality[0] / 4, self.quality[1] / 4 * 1 + 7
        ...
        self.background_surf = pygame.transform.scale(self.background_surf, self.quality)
        self.background_rect = self.background_surf.get_rect()
        self.background_surf.set_alpha(100)  # change opacity

        # Set Footer
        self.footer_surf = pygame.Surface((self.quality[0], self.quality[1] / 10 * self.gui_scale))
        pos = (self.quality[0] / 2, self.quality[1])
        ...
        self.surface_size = (self.quality[0], title_height + options_height)
    ...
    def text_n_animation(self):
        ...
        for index in self.menu_options:
            ...
            element_height = self.quality.y / len(self.menu_options) + self.spacing
    ...
    def rescale_n_draw_surfaces(self):
        ...
        pos = (self.quality[0] / 2, 0 + self.scroll_offset)  # stands for game canvas center
    ...
    def save(self):
        ...
        self.game.screen_resolution = self.resolution
        self.game.game_resolution = self.quality

        # self.game.game_resolution = self.quality  # temp
...
# -----------------------------------------------------------------------------------------------
    ...
    def update(self, delta_time, actions):
        ...
        # Scroll wheel
        if self.surface_rect.h + self.footer_rect.h >= self.quality.y:
            if self.game.mwheel.y:
                self.scroll_offset += self.game.mwheel.y * 20
                print(self.scroll_offset)
                if self.scroll_offset > 0:
                    self.scroll_offset = 0
                if self.scroll_offset < -self.surface_rect.h + self.quality.y - self.footer_rect.h:
                    self.scroll_offset = -self.surface_rect.h + self.quality.y - self.footer_rect.h
    ...
    def transition_state(self):
        if self.menu_options[self.index] == "Aspect Ratio":
            # new_state = GameWorld(self.game)
            # new_state.enter_state()
            pass
        elif self.menu_options[self.index] == "Resolution":
            pass  # TODO
        elif self.menu_options[self.index] == "Full Screen":
            pass  # TODO
        elif self.menu_options[self.index] == "Target FPS":
            pass  # TODO
        elif self.menu_options[self.index] == "HUD Scale":
            pass  # TODO
