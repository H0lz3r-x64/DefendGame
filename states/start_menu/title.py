...
from states.world.game_world import GameWorld
from states.start_menu.settings import Settings
from states.start_menu.exit import Exit
...
class Title(State):
    def __init__(self, game):
        ...
        self.menu_options = {0: "Start", 1: "Settings", 2: "Credits", 3: "Exit"}
        self.index = 0
    ...
    def __create_menu_options__(self):
        ...
        self.background_surf = pygame.transform.scale(self.background_surf, (self.game.game_resolution[0], self.game.game_resolution[1]))
        self.background_rect = self.background_surf.get_rect()
        self.background_surf.set_alpha(100)
        # Set the menu title
        self.title_surf = pygame.surface.Surface((self.game.screen_resolution[0], self.game.screen_resolution[1] / 4 * 1))
        ...
        self.cursor_pos_x, self.cursor_pos_y = self.game.game_resolution[0]/4, self.game.game_resolution[1]/4*1 + 7
        self.cursor_rect.center = self.cursor_pos_x, self.cursor_pos_y

    def draw_menu_options(self):
        x = self.menu_rect.width / 2
        y = 15
        self.game.draw_text(self.title_surf, "Game States", (0, 188, 238), x, y, "custom1 60")
    ...
    def update_cursor(self, actions):
        ...
        if actions["down"]:
            ...
            self.index = (self.index - 1) % len(self.menu_options)
        self.cursor_rect.y = self.cursor_pos_y + (self.index * 70)
